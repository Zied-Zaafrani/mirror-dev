"""Create the Phase 7 MedGCN Aux Loss Kaggle notebook."""
import json
...
cells.append(md_cell(
    "# MIRROR — Phase 7: MedGCN Auxiliary Lab Imputation Loss\n\n"
    "**Objective:** Ablate the MedGCN approach of adding an auxiliary imputation loss for missing labs "
    "to force the latent lab representation to retain semantic meaning.\n\n"
    "**Configs:**\n"
    "- `baseline_per_lab`: Standard `per_lab_attn` without imputation loss\n"
    "- `aux_impute_l0.1`: `per_lab_attn` with `--use_lab_impute_loss` and `--lambda_lab 0.1`\n"
    "- `aux_impute_l0.5`: `per_lab_attn` with `--use_lab_impute_loss` and `--lambda_lab 0.5`\n"
    "- `aux_impute_l1.0`: `per_lab_attn` with `--use_lab_impute_loss` and `--lambda_lab 1.0`\n"
))
...
cells.append(code_cell(
    ...
    "    ('--ablation no_ablation --lab_encoder_type per_lab_attn --use_lab_impute_loss --lambda_lab 0.1', 'aux_impute_l01'),\n"
    "    ('--ablation no_ablation --lab_encoder_type per_lab_attn --use_lab_impute_loss --lambda_lab 0.5', 'aux_impute_l05'),\n"
    "    ('--ablation no_ablation --lab_encoder_type per_lab_attn --use_lab_impute_loss --lambda_lab 1.0', 'aux_impute_l10'),\n"
    "]\n"
))
...
out = Path("notebooks/train_kaggle_phase7_aux_loss.ipynb")
