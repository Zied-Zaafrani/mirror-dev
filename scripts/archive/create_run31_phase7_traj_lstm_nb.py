"""Create the Phase 7 Lab Trajectory LSTM Kaggle notebook."""
import json
...
cells.append(md_cell(
    "# MIRROR — Phase 7: Lab Trajectory LSTM\n\n"
    "**Objective:** Ablate the use of a shared LSTM to process the entire temporal sequence "
    "of each lab independently, extracting trajectory features rather than just current values.\n\n"
    "**Configs:**\n"
    "- `baseline_per_lab`: Standard `per_lab_attn` without trajectory\n"
    "- `traj_lstm`: `traj_lstm` encoder using the full patient lab history\n"
))
...
cells.append(code_cell(
    ...
    "    ('--ablation no_ablation --lab_encoder_type traj_lstm --use_lab_trajectory', 'traj_lstm'),\n"
    "]\n"
))
...
out = Path("notebooks/train_kaggle_phase7_traj_lstm.ipynb")
