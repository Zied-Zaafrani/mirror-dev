...
# --- SHAPE Recurrent Attention Block (RAB) Components ---
...
class BlockRecurrentAttention(nn.Module):
    def __init__(self, dim: int, dim_state: int, dim_head: int = 64, state_len: int = 512, heads: int = 8, **kwargs):
        ...
        self.input_self_attn = Attention(dim, heads = heads, causal = True)
        self.state_self_attn = Attention(dim_state, heads = heads, causal = False)

        self.input_state_cross_attn = Attention(dim, heads = heads, causal = False)
        self.state_input_cross_attn = Attention(dim_state, heads = heads, causal = False)
...
class ISAB(nn.Module):
    ...
    def forward(self, input_tuple):
        ...
        return self.mab1(X, H, None)

# --- Wrappers for MIRROR ---

@TEMPORAL_ENCODERS.register("rab")
class SHAPEEncoder(nn.Module):
    """SHAPE Block Recurrent Attention Encoder wrapper."""
    def __init__(self, hidden_dim, state_len=64, heads=4, **kwargs):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.state_len = state_len
        self.heads = heads
        self.rab = BlockRecurrentAttention(
            dim=hidden_dim, 
    ...
    def forward(self, x, lengths, **kwargs):
        ...
        if not hasattr(self, "_logged_flow"):
            logger.info(f"  [Temporal] SHAPE-RAB Backbone Active | state_len={self.state_len} | heads={self.heads}")
        ...
        out, next_state = self.rab(x, state=None, mask=mask)
            
        return out

@TEMPORAL_ENCODERS.register("isab_rab")
class ISABSHAPEEncoder(nn.Module):
    """SHAPE Encoder with ISAB compression."""
    def __init__(self, hidden_dim, isab_inds=4, state_len=64, heads=4, **kwargs):
        super().__init__()
        self.isab_inds = isab_inds
        self.state_len = state_len
        self.heads = heads
        self.isab = ISAB(hidden_dim, hidden_dim, heads, isab_inds, ln=True)
        self.rab = BlockRecurrentAttention(
            dim=hidden_dim, 
            dim_state=hidden_dim, 
            dim_head=hidden_dim//heads,
    ...
    def forward(self, x, lengths, **kwargs):
        ...
        """
        if not hasattr(self, "_logged_flow"):
            logger.info(f"  [Temporal] ISAB-SHAPE Active | inds={self.isab_inds} | state_len={self.state_len}")
            self._logged_flow = True
        ...
        if lengths is not None:
            mask = torch.arange(T, device=device).unsqueeze(0) >= lengths.unsqueeze(1)
            mask_isab = mask.unsqueeze(1).expand(-1, self.isab.num_inds, -1)
            mask_isab = mask_isab.repeat(self.isab.mab0.num_heads, 1, 1)
        else:
            mask = None
            mask_isab = None
            
        # ISAB pass
        x = self.isab((x, mask_isab))
